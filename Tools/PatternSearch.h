...
// Convert a pattern string like "48 8B ?? ??" into a byte sequence.
...
// Overload that accepts a pattern in string form with wildcards.
inline uintptr_t FindPattern(const wchar_t* module, const std::string& pattern)
{
    return FindPattern(module, PatternToBytes(pattern));
}

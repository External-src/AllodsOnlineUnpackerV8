#include "CreatureEffectsOnTarget.h"

CreatureEffectsOnTarget::CreatureEffectsOnTarget(DWORD struct_addr, string name, XDocument * d)
{
	doc = d;
	root = doc->NewElement(name.c_str());
	main = struct_addr;
	AddAttribute("type", "client.Replica.CreatureEffectsOnTarget");
	Init();
}

CreatureEffectsOnTarget::~CreatureEffectsOnTarget()
{
}

void CreatureEffectsOnTarget::Init()
{
	...
	AddText("direction", Enums::GetDirection(Read(0x40)));
}